...
import os, time, threading, logging, requests, schedule, re, json
...
YIELD_FILE = "/tmp/yields.json"

def save_yields():
    """Simpan yields dan sources ke file supaya tidak hilang saat restart."""
    try:
        data = {"yields": YIELDS, "sources": SOURCES, "updated_at": UPDATED_AT}
        with open(YIELD_FILE, "w") as f:
            json.dump(data, f)
        log.info("Yields disimpan ke file.")
    except Exception as e:
        log.error(f"save_yields error: {e}")

def load_yields():
    """Load yields dari file saat startup."""
    global YIELDS, SOURCES, UPDATED_AT
    try:
        if os.path.exists(YIELD_FILE):
            with open(YIELD_FILE, "r") as f:
                data = json.load(f)
            YIELDS = data.get("yields", YIELDS)
            SOURCES = data.get("sources", SOURCES)
            UPDATED_AT = data.get("updated_at", UPDATED_AT)
            log.info(f"Yields dimuat dari file: {UPDATED_AT}")
        else:
            log.info("File yields tidak ada, pakai fallback.")
    except Exception as e:
        log.error(f"load_yields error: {e}")
...
def get_yields_auto():
    ...
    for country, tenors in FETCH.items():
        for tenor, func in tenors.items():
            val = func()
            # Skip kalau sudah diupdate manual
            if SOURCES.get(tenor, {}).get(country) == "manual":
                log.info(f"  {country} {tenor}: skip (manual)")
                continue
...
def run_yield(chat_id=CHAT_ID, thread_id=THREAD_ID, tenor="2Y"):
    send_message(f"⏳ Mengambil data FX...\nHarap tunggu ~10 detik.", chat_id, thread_id)
    try:
        fx = get_all_fx()
...
def polling_loop():
    ...
    load_yields()  # Load yield tersimpan saat startup
